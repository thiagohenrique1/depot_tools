...
class RelativeRoot:
  ...
  @classmethod
  def create(cls, checkout_dir: Path, name: str) -> 'RelativeRoot':
...
class Result:
  ...
  def __init__(self, *, checkout_dir: Path, source_root: RelativeRoot,
               patch_root: RelativeRoot | None = None,
               properties: dict[str, str],
               manifest: dict[str, ManifestRepo],
               fixed_revisions: dict[str, str],
               out_commit: common_pb2.GitilesCommit | None):
    object.__setattr__(self, 'checkout_dir', checkout_dir)
    object.__setattr__(self, 'source_root', source_root)
    object.__setattr__(self, 'patch_root', patch_root)
    object.__setattr__(self, 'properties', properties)
    object.__setattr__(self, 'manifest', manifest)
    object.__setattr__(self, 'fixed_revisions', fixed_revisions)
    object.__setattr__(self, 'out_commit', out_commit)
...
class _EnabledTurboCiCheckHandler(_TurboCICheckHandler):
  ...
  def set_gerrit_change(self, gerrit_change: common_pb2.GerritChange,
    ...
    self._source_check_options.gerrit_changes.add(
        ...
        hostname=_remove_suffix(gerrit_change.host,
  ...
  def set_result(self, result: Result) -> None:
    ...
    if self._gerrit_change:
      gob_source_check_results.changes.add(
          ...
          host=_remove_suffix(self._gerrit_change.host,
                              '-review.googlesource.com'),
          project=self._gerrit_change.project,
          branch=_remove_prefix(
              self._api.m.tryserver.gerrit_change_target_ref, 'refs/heads/'),
